...
class MVSSystem(LightningModule):
    def __init__(self, args):
        ...
        # Create model
        self.nerf, self.unet, self.queries_attention, self.grad_vars = create_model(args, device)
    ...
    def training_step(self, batch, batch_nb):
        """
        imgs        : [B, N, 3, H, W]
        proj_mats   : [B, N, 3, 4]      - w2p
        c2ws, w2cs  : [N, 4, 4]         - Extrinsic
        intrinsics  : [N, 3, 3]
        """
        ...
        imgs, proj_mats = data_mvs['images'], data_mvs['proj_mats']         # [B, N, 3, H, W]
        near_fars, depths_h = data_mvs['near_fars'], data_mvs['depths_h']   # [B, N, 2], [B, N, 1, 1]
        
        N_rays, N_samples = args.batch_size, args.N_samples
        c2ws, w2cs, intrinsics = pose_ref['c2ws'], pose_ref['w2cs'], pose_ref['intrinsics']

        # 1. Feature map 만들기 (imgs -> feat)
        feat_map1, feat_map2 = self.unet(imgs[0, :1]), self.unet(imgs[0, 1:])
        feat_map1, feat_map2 = self.queries_attention(feat_map1, feat_map2)
        feat_map = torch.stack([feat_map1, feat_map2], dim=0)
        
        # 2. Ray sampling
        imgs = self.unpreprocess(imgs)
        rays_os, rays_ds, viewdirs, colors, feats = build_rays(imgs, c2ws, w2cs, intrinsics, N_rays, feat_map)
        
        # 3. 해당 Feature 와 x, d 등을 활용하여 MLP 학습하기
        N = imgs.shape[1] # =2
        
        all_comp_rgbs, all_distances, all_accs = [], [], []
        for n in range(N):
            rays_o = rays_os[n]     # [N_rays, 3]
            rays_d = rays_ds[n]     # [N_rays, 3]
            viewdir = viewdirs[n]   # [N_rays, 3]
            near, far = near_fars[0, n, :1], near_fars[0, n, 1:] # [N_rays, 1]
            feat = feats[n]         # [N_rays, E]
            color = colors[n]       # [N_rays, 3]          

            n_view_comp_rgbs, n_view_distances, n_view_accs = [], [], []
            for i in range(0, N_rays, self.chunk):
                comp_rgbs, distances, accs = self.nerf(
                    rays_o[i:i+self.chunk], rays_d[i:i+self.chunk], 
                    near[i:i+self.chunk], far[i:i+self.chunk], viewdir[i:i+self.chunk], 
                    feat[i:i+self.chunk], color[i:i+self.chunk])
                
                n_view_comp_rgbs.append(comp_rgbs)     # [2, N_rays, 3]
                n_view_distances.append(distances)     # [2, N_rays, 1]
                n_view_accs.append(accs)               # [2, N_rays, 1]

            n_view_comp_rgbs = torch.cat(n_view_comp_rgbs, 1) # [2, N_rand, 3]
            n_view_distances = torch.cat(n_view_distances, 1) # [2, N_rand, 1]
            n_view_accs = torch.cat(n_view_accs, 1)           # [2, N_rand, 1]

            all_comp_rgbs.append(n_view_comp_rgbs)     # [N, 2, N_rays, 3]
            all_distances.append(n_view_distances)     # [N, 2, N_rays, 1]
            all_accs.append(n_view_accs)               # [N, 2, N_rays, 1]

        all_comp_rgbs = torch.stack(all_comp_rgbs, 0)   # [N, 2, N_rays, 3]
        all_distances = torch.stack(all_distances, 0)   # [N, 2, N_rays, 1]
        all_accs = torch.stack(all_accs, 0)             # [N, 2, N_rays, 1]
