...
def train(rank, world_size, args):
    ...
    print("Train start...!")
    with tqdm(len(train_dataloader) * epochs) as pbar:
        for epoch in range(epochs):
            ...
            for step, (model_input, gt) in enumerate(train_dataloader):
                ...
                with torch.no_grad():
                    ...
                    if rank == 0:
                        rgb = model_output_full['rgb'].reshape(1, 1, H, W, 3) # [1, 1, HW, 3]
                        rgb8 = to8b(rgb[0][0].cpu().numpy())
                        filename = os.path.join(output_dir, args.expname, '{:03d}_{:06:d}.png'.format(epoch, total_steps))
