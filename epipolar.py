...
def project_rays(
    ...
    intrinsics = intrinsics[..., :3, :3]        # [2B, 3, 3]

    # Transform the rays into camera space.
    world_to_cam = torch.linalg.inv(extrinsics) # extrinsics c2w 
    origins = homogenize_points(origins)        # [2B, 1024, 4(xyz1)] 
    origins = einsum(world_to_cam, origins, "c i j, c r j -> c r i")    # [2B, 4, 4] [2B, 1024, 4] = [2B, 1024, 4]
    directions = homogenize_vecs(directions)    # [2B, 1024, 4(xyz0)] 
    directions = einsum(world_to_cam, directions, "c i j, c r j -> c r i") # [2B, 1024, 4]
    origins = origins[..., :3]          # [2B, 1024, 3]
    directions = directions[..., :3]    # [2B, 1024, 3]
